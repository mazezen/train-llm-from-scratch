...
class ModelArguments:
    ...
    config_name: Optional[str] = field(
        default=None, metadata={
            "help": "预训练使用, Config 文件地址"
        },
    )
    tokenizer_name: Optional[str] = field(
        default=None, metadata={"help": "预训练 Tokerizer 地址"}
    )
...
def main():
    ...
    # 初始化 SwanLab
    swanlab.init(project="pretrain", experiment_name="from_scrach", mode="local")
    ...
    # 训练整体情况记录
    logger.warning(
        f"Prcess rank: {training_args.local_rank}, device: {training_args.device}, n_gpu: {training_args.n_gpu}"
    ...
    if model_args.config_name is not None:
        ...
        logger.info(f"预训练一个新的模型 - Total size = {n_params/2**20:.2f}M params")
    ...
    def group_texts(examples):
        # 将文本拼接起来
        concatedated_examples = {k: list(chain(*examples[k])) for k in examples.keys()}
        # 计算拼起来的整体长度
        total_length = len(concatedated_examples[list(examples.keys())[0]])
        # 如果长度太长, 进行分块
        if total_length >= block_size:
            total_length = (total_length // block_size) * block_size
        result = {
            k: [t[i : i + block_size] for i in range(0, total_length, block_size)]
            for k, t in concatedated_examples.items()
        }
